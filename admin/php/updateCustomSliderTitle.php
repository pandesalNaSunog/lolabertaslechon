...
if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && $_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest'){
    ...
    $con = connect();
        ...
        if($mainRow = $main->fetch_assoc()){
            $query = $con->prepare("UPDATE custom_slider_title SET slider_title = ? ,updated_at= ?");
            $query->bind_param("ss", $title, $today);
            $query->execute();
        }else{
            $query = $con->prepare("INSERT INTO custom_slider_title(slider_title,created_at,updated_at) VALUES (?,?,?)");
